...
wait_for_umc () {  # <ip>
	...
	do
		...
		sleep 60
		if curl --fail --silent "http://$1/univention/management/" >/dev/null 2>&1 && curl --fail --silent "http://$1/univention/get/modules" >/dev/null 2>&1
		then
...
write_slave1_preseed () {
	local host suite dir
	host="$(ucr get ldap/master)"
	suite="ucs$(ucr get version/version|tr -d .)$(ucr get version/patchlevel)"
	dir="$(echo '/univention-repository/@%@version/version@%@/maintained/@%@version/version@%@-@%@version/patchlevel@%@'|ucr filter)"
...
d-i ucr/interfaces/ens8/type string manual
d-i ucr/interfaces/ens8/start string true
