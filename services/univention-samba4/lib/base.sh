...
extract_binddn_bindpwd_bindpwdfile_dcaccount_authenticationfile_from_args() {
	...
	test -z "$authenticationfile" && authenticationfile="/var/run/univention-join/samba-authentication-file"
}
...
is_localhost_administration() {
	...
	ldif=$(univention-ldapsearch -H "ldap://$master:$master_port" -LLL \
